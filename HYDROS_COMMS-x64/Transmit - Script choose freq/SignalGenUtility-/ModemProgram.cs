...
namespace DelsysSigNIalGen
{
    class ModemProgram
    {
        ...
        public static (NetworkStream, NetworkStream, TcpClient, TcpClient) ModemInit()
        {
            ...
            string ipAddress = "172.17.251.198";
            int port = 19000;
            int pcmPort = 19003;
        ...
        public static void RunModemProgram(int heart_rate, int spo2, NetworkStream stream, NetworkStream pcmStream, int calls, BlockingCollection<float[]> waveBuff, int scf, float voltAmp)
        {
            ...
            byte[] responseData = new byte[6400 * 4];
            float[] resArray = new float[1024000 * 5]; // will get exactly 5s in return
            ...
            scaleVoltage(cutArray, waveBuff, voltAmp);
        }
        ...
        public static void scaleVoltage(float[] sigin, BlockingCollection<float[]> waveBuff, float voltAmp)
        {
                float[] sigout = new float[sigin.Count()];
                // Define input range
                float inputMin = sigin.Min();
                float inputMax = sigin.Max();
