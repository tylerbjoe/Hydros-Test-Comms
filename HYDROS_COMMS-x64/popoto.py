...
#         WAVE_FILE_SUPPORTED = False
# else:
#     print('wave file operations not supported')
...
class popoto:
    ...
    def getExclusiveAccess(self):
        ...
        while accessGranted is False:
            ...
            while tokenReplyReceived is False:
                if waitingCounter == 5:
                    ...
                    if reply:
                        if "ExclusivityToken" in reply:
                            if token in reply["ExclusivityToken"]:
                                ...
                                self.logger.info(
                                    "Waiting for ExclusivityToken; Process {} has it".format(reply["ExclusivityToken"]))
                                time.sleep(2)
                                tokenReplyReceived = True
                                waitingCounter += 1
                except:
                ...
                if len(data) >= 1:
                    ...
                    if ord(data) != 13:
                        ...
                        try:
                            ...
                            if "RemoteCommand" in reply and self.remoteCommandHandler is not None:
                            ...
                            if jsonData == '{"BatteryVoltage":-0.001080}':
                                ...
                                self.logger.warning(
                                    "(This may be due to a missing hardware component or firmware update.")
    ...
    def getCycleCount(self):
        while self.replyQ.empty() is False:
            self.replyQ.get()
    ...
    def dispMips(self, mips):
        v = {}
        print('Name                            |       min  |        max |     total  |      count |    average |  '
              'peak mips | avg mips')
        for module in mips:
            v = mips[module]
            name = module
            print('{:<32}|{:12}|{:12}|{:12.1e}|{:12}|{:12.1f}|{:12.1f}|{:12.1f}'.format(name,
                                                                                        v['min'], v['max'],
                                                                                        v['total'],
                                                                                        v['count'],
                                                                                        v['total'] / v['count'],
    ...
    ''' This is the version I edited for analyzing the results of pool tests '''

    def pumpAudio(self, inFilename):
        if not os.path.exists("/etc/PopotoSerialNumber.txt"):  # Do not execute on modem hardware
            WAVE_FILE_SUPPORTED = True
        ...
        if WAVE_FILE_SUPPORTED is False:
        ...
        print("Processing input wave file {}".format(inFilename))
        ...
        if self.pcmiosocket is not None:
            ...
            while done is False:
                ...
                for sock in readable:
                    ...
                    # need to manually add padding for end the file.
                    # The above while loop will only pull data from the socket
                    if len(data) < FrameSize:
                        diff = FrameSize - len(data)
                        data += "\0" * diff
            ...
            # '''  This is the original version from Popoto. Needed to create pure encoded signals'''
    # def pumpAudio(self, inFilename, outFilename):

    #     # print(os.path.exists("/etc/PopotoSerialNumber.txt"))
