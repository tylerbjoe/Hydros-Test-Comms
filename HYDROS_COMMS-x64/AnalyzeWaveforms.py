...
### Load in csv
filePath = "C:/Users/AEngel/Documents/laura_testing_things/testingPackets" ## Change name
fc_water = 100_000 ## Change carrier freq
fs_water = 1_000_000
fs_modem = 102_400
fc_modem = 35_000

data = pd.read_csv(filePath + ".csv")
print(f"Length of csv: {len(data)}, ({len(data) / fs_water} seconds)")
...
###### Plot psd
plot_psd(data,fs_water)
plt.show()

###### Downsample and downshift
down_sig = downsample_downshift(data, fc_water, fs_water, fs_modem, fc_modem)
print(f"Length of downsampled sigal: {len(down_sig)}, ({len(down_sig) / fs_modem} seconds)")

###### Plot spectogram
plot_spectrogram(down_sig, fs_modem)
plt.show()

###### Save as .wav file
down_sig_int = np.int16(down_sig * 32767) # Convert to non-float formatting
scipy_io_wavfile.write((filePath+".wav"), fs_modem, down_sig_int)
print("Saved wavefile to " + filePath + ".wav")